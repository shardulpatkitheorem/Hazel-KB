...
def main() -> int:
    ...
    def same(a: str | None, b: str) -> bool:
        ...
        Git checks this repository out as CRLF on Windows and LF on Linux, so a
        byte comparison would report stale on one platform and current on the
        other for identical content. The generator always writes LF; the check
        must not care what the working copy was checked out as.
        """
        if a is None:
            return False
        return a.replace("\r\n", "\n") == b.replace("\r\n", "\n")
    ...
    for path, content in targets:
        ...
        if args.check:
            ...
            # newline="" plus explicit \n: the generated bytes must be identical
            # on every platform, or --check fails on CI while passing locally.
